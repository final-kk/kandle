/**
 * Contiguity Utilities for WebGPU Backend
 *
 * 提供 tensor 连续性检查和自动克隆功能
 * 用于需要连续内存布局的 kernel
 */
...
/**
 * 检查 tensor 是否内存连续（offset == 0 且 strides 是标准 row-major）
 */
export function isTensorContiguous(tensor: ITensorHandle): boolean {
    return tensor.offset === 0 && isContiguousStrides(tensor.shape, tensor.strides);
}

/**
 * 确保 tensor 连续，如不连续则创建连续副本
 *
 * @param tensor 输入 tensor
 * @param label 可选的标签用于调试日志
 * @returns 连续的 tensor（可能是原 tensor 或新创建的副本）
 */
export function ensureContiguous(
    tensor: ITensorHandle,
    label?: string
): ITensorHandle {
    if (isTensorContiguous(tensor)) {
        return tensor;
    }

    // 执行 strided -> contiguous copy
    return executeStridedToContiguousCopy(tensor);
}

/**
 * 执行 strided tensor 到 contiguous tensor 的拷贝
 *
 * 这是一个通用的轻量级拷贝函数，避免了 TensorIterator 的复杂依赖。
 */
export function executeStridedToContiguousCopy(
    ...
    const pipelineKey = `utils.strided_copy.r${rank}.${dtype}`;
...
function buildStridedCopyShader(rank: number, dtype: string): string {
    ...
    return `
// Strided to Contiguous Copy Shader (General Utility)
